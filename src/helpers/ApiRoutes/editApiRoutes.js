...
const fakeBackend = () => {
  ...
  mock.onPost(url.POST_FAKE_LOGIN).reply(async (config) => {
    ...
    mock.onPut(url.UPDATE_CUSTOMER_STATUS).reply(async (config) => 
    {
        const user = JSON.parse(config["data"]);
        console.log("user", user);
        
        // Make changes to the user's status here
        ...
        return new Promise((resolve, reject) => {
          setTimeout(() => {
            if (user.status === "inactive") {
              resolve([200, { success: true, data: user }]);
            } else {
              reject({ message: "Failed to update customer status." });
            }
          }, 2000); // Delay for 2 seconds to simulate asynchronous behavior
        });
      });
