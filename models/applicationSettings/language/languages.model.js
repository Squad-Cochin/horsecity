  ...
  static async addNewLanguage(requestBody, file) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        requestBody.abbreviation = requestBody.abbreviation
          .toLowerCase()
          .trim();
        const { name, abbreviation } = requestBody;

        let uploadLanguage = await commonoperation.fileNameUpload(
          file?.language_file
        );
        if (uploadLanguage.message == "INVALIDFORMAT") {
          ...
          let insQuery = `INSERT INTO ${
            constants.tableName.languages
          } (name, file,abbreviation, created_at)
        VALUES ('${name}','${uploadLanguage}','${abbreviation}','${time.getFormattedUTCTime(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (uploadLanguage.message == invalidFormat) {
          ...
          uploadLanguage.message == invalidAttachment ||
          uploadLanguage
        ) {
          const { name, abbreviation } = requestBody;
