...
module.exports = class serviceProviders {
  ...
  static async addNewServiceProviders(requestBody, file) {
    return new Promise(async (resolve, reject) => {
      try {
        let uploadAttachment = await commonoperation.fileUpload(
          file,
          constants.attachmentLocation.serviceProvider.licenceImage.upload
        );
        ...
        if (uploadAttachment.message == invalidFormat) {
          ...
          } (name, email, user_name, password,role_Id , contact_person, contact_no, contact_address, emergency_contact_no, licence_image, licence_no, expiry_at, created_at)
            VALUES ('${name}', '${email}', '${user_name}', '${await commonoperation.changePasswordToSQLHashing(
            password
          )}','${role_id}', '${contact_person}', '${contact_no}', '${contact_address}', '${emergency_contact_no}', '${uploadAttachment}', '${licence_no}', '${time.addingSpecifiedDaysToCurrentDate(
            constants.password.expiry_after
  ...
  static async updateServiceProvider(requestBody, file, id) {
    return new Promise(async (resolve, reject) => {
      try {
        let uploadLicence = await commonoperation.fileUpload(
          file?.licence_image,
          constants.attachmentLocation.serviceProvider.licenceImage.upload
        );
        const invalidFormat = "INVALIDFORMAT";
        ...
        if (uploadLicence.message == invalidFormat) {
          ...
          uploadLicence.message == invalidAttachment ||
          uploadLicence
        ) {
          const {
            name,
        ...
        user_name = '${user_name}',
        ${
          constants.Roles.service_provider == service_provider_id
            ? ""
            : `role_Id = ${role_id},`
        }
        ...
        ${
          uploadLicence.message === invalidAttachment
            ? ""
            : `licence_image = '${uploadLicence}',`
        } 
        licence_no = '${licence_no}',
        expiry_at = '${time.addingSpecifiedDaysToCurrentDate(
          constants.password.expiry_after
        )}',
  ...
  static async getSpDrivers(spID) {
    return new Promise((resolve, reject) => {
      try {
        ...
        console.log(
          "Error while feching particular service provider drivers",
          err
        );
      }
    });
