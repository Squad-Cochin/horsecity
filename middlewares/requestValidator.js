...
exports.VerifyRoleId = async (req, res, next) =>{
    ...
    if (!role_id) {
        ...
        let verifyRoleId = await commonfetching.dataOnCondition(constants.tableName.roles,role_id,'id')
            if(verifyRoleId === 'err'){
                return res.status(200).send({
                    code: 500,
                    success: false,
...
{
    const { customer_id,vehicle_id,driver_id,service_provider_id,drop_date} = req.body;
    if(!customer_id){
        return res.status(200).send
        ({ 
            code: 400,
        ...
        //             code: 400,
        //             success: false,
        //             message: 'Please go and select vehicle.'
        //         });
        //  }else{
            return res.status(200).send
            ({ 
        ...
        //             code: 400,
        //             success: false,
        //             message: "Please go and select driver."    
        //         });
        //   }else{
                return res.status(200).send
                ({ 
        ...
        let verifycustomerId = await commonfetching.dataOnCondition(constants.tableName.customers,customer_id,'id');
        if(verifycustomerId.length != 0){
            if(!verifycustomerId[0].deleted_at){
                if(verifycustomerId[0].status=== constants.status.active){
                    ...
                    let verifyServiceProviderId = await commonfetching.dataOnCondition(constants.tableName.service_providers,service_provider_id,'id');
                    if(verifyServiceProviderId.length != 0){
                        if(!verifyServiceProviderId[0].deleted_at){
                            if(verifyServiceProviderId[0].status=== constants.status.active){

                    /*******Verification for vehicle******* */
                    let verifyVehicleId = await commonfetching.dataOnCondition(constants.tableName.vehicles,vehicle_id,'id');
                    if(verifyVehicleId.length != 0){
                        if(!verifyVehicleId[0].deleted_at){
                            if(verifyVehicleId[0].status=== constants.status.active){
                                ...
                                let verifyDriverId = await commonfetching.dataOnCondition(constants.tableName.vehicles,vehicle_id,'id');
                                if(verifyDriverId.length != 0){
                                    if(!verifyDriverId[0].deleted_at){
                                        if(verifyDriverId[0].status=== constants.status.active){
